...
{
    public partial class IU_InfoEnvio : Form
    {
        ...
        public IU_InfoEnvio(List<Entrega> entrega, List<DetalleLogistica> detalles)
        {
            InitializeComponent();
        ...
        private void cargarDetalles(List<DetalleLogistica> detalles, List<Entrega> entregaDatos)
        {
            ...
            {
                _cliente[contadorIndice].Text = item.NombreCliente;
                _fechaEntrega[contadorIndice].Text = item.FechaEntrega.ToShortDateString();
                _horaInicio[contadorIndice].Text = item.HoraEntregaDesde.ToShortTimeString() + " hs";
                _horaFin[contadorIndice].Text = item.HoraEntregaHasta.ToShortTimeString() + " hs";
                _calle[contadorIndice].Text = item.NombreCalle + " " + item.NumeroCalle + ", " + item.NombreBarrio + "  " + item.NombreLocalidad;
                _telefono[contadorIndice].Text = item.NumeroTelefono;
                _precio[contadorIndice].Text = item.PrecioEntrega.ToString("$ 0.00");
                _observacion[contadorIndice].Text = item.Descripcion;

                foreach (var item2 in detalles)
                {
                    if (item.CodigoEntrega == item2.CodigoEntrega)
                    {
                        articulo.mostrarDatos(item2.CodigoArticulo);
                        tabControl1.SelectedIndex = contadorIndice;
                        _listaDGV[contadorIndice].Rows.Add(item2.CodigoArticulo, articulo.Descripcion, item2.Cantidad);
                    }
                }
                contadorIndice++;
            }
